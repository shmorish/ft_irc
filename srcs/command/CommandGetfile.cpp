...
void Command::getfile() {
    try {
        ...
        if (file == NULL)
            throw runtime_error(err_825(_user, filename));
        if (_user.get_fd() != file->get_accepter_fd())
            throw runtime_error(err_828(_user, filename));
        // 置きたい場所のpathが正しいかどうか、ディレクトリか確認
        string get_path = _parser.get_args().at(3);
        if (get_path.empty())
            throw runtime_error(err_461(_user, get_path));
        int fd = open(get_path.c_str(), O_DIRECTORY);
        if (fd == -1)
            throw runtime_error(err_829(_user, get_path));
        close(fd);
        // ファイルを受け取る
        ...
        if (access(send_path.c_str(), F_OK) == 0)
            throw runtime_error(err_826(_user, send_path));
        // ファイルを作成権限があるか確認
        fstream ofs(send_path, ios::out);
        if (ofs.fail())
            throw runtime_error(err_827(_user, send_path));
        fstream ifs(file->get_send_path(), ios::in);
        if (ifs.fail())
            throw runtime_error(err_827(_user, file->get_send_path()));
